#include "boid_rigid_body2d.hpp"

#include <godot_cpp/classes/shape2d.hpp>

namespace godot {
BoidRigidBody2D::BoidRigidBody2D() {}

BoidRigidBody2D::~BoidRigidBody2D() {}

void BoidRigidBody2D::_process(double delta) {
    mTimeEmit += delta;
...
void BoidRigidBody2D::_enter_tree() {
...
void BoidRigidBody2D::_exit_tree() {
    mBoidManager = nullptr;
    mCluster     = nullptr;
}

void BoidRigidBody2D::_draw() {
    ...
    return RigidBody2D::_draw();
}

void BoidRigidBody2D::_physics_process(double delta) {
    if (mCluster != nullptr) {
        auto force = mCluster->applyRules(this);
        apply_force(force * delta * mCluster->gameTimeFrameRate());
    }
}

void BoidRigidBody2D::_bind_methods() {
    ClassDB::bind_method(D_METHOD("updateManager", "manager"), &BoidRigidBody2D::updateManager);
    ClassDB::bind_method(D_METHOD("updateBoidCluster", "cluster"), &BoidRigidBody2D::updateBoidCluster);
...
#define PROPERTY_BINDER(type, name, getter, setter)                                                                    \
    ClassDB::bind_method(D_METHOD(#getter), &BoidRigidBody2D::name);                                                   \
    ClassDB::bind_method(D_METHOD(#setter, #name), &BoidRigidBody2D::setter);
