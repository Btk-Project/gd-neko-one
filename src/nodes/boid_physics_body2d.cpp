#include "boid_physics_body2d.hpp"

namespace godot {
BoidPhysicsBody2D::BoidPhysicsBody2D() {}

BoidPhysicsBody2D::~BoidPhysicsBody2D() {}

void BoidPhysicsBody2D::_process(double delta) {
    mTimeEmit += delta;
...
void BoidPhysicsBody2D::_enter_tree() {
...
void BoidPhysicsBody2D::_exit_tree() {
    mBoidManager = nullptr;
    mCluster     = nullptr;
}

void BoidPhysicsBody2D::_draw() {
    ...
    return PhysicsBody2D::_draw();
}

void BoidPhysicsBody2D::_physics_process(double delta) {
    if (mCluster != nullptr) {
...
void BoidPhysicsBody2D::_bind_methods() {
    ClassDB::bind_method(D_METHOD("updateManager", "manager"), &BoidPhysicsBody2D::updateManager);
    ClassDB::bind_method(D_METHOD("updateBoidCluster", "cluster"), &BoidPhysicsBody2D::updateBoidCluster);
...
#define PROPERTY_BINDER(type, name, getter, setter)                                                                    \
    ClassDB::bind_method(D_METHOD(#getter), &BoidPhysicsBody2D::name);                                                 \
    ClassDB::bind_method(D_METHOD(#setter, #name), &BoidPhysicsBody2D::setter);
