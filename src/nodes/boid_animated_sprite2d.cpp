#include "boid_animated_sprite2d.hpp"

#include <godot_cpp/classes/shape2d.hpp>

namespace godot {
BoidAnimatedSprite2D::BoidAnimatedSprite2D() {}

BoidAnimatedSprite2D::~BoidAnimatedSprite2D() {}

void BoidAnimatedSprite2D::_process(double delta) {
    mTimeEmit += delta;
...
void BoidAnimatedSprite2D::_enter_tree() {
...
void BoidAnimatedSprite2D::_exit_tree() {
    mBoidManager = nullptr;
    mCluster     = nullptr;
}

void BoidAnimatedSprite2D::_draw() {
    ...
    return AnimatedSprite2D::_draw();
}

void BoidAnimatedSprite2D::_physics_process(double delta) {
    if (mCluster != nullptr) {
...
void BoidAnimatedSprite2D::_bind_methods() {
    ClassDB::bind_method(D_METHOD("updateManager", "manager"), &BoidAnimatedSprite2D::updateManager);
    ClassDB::bind_method(D_METHOD("updateBoidCluster", "cluster"), &BoidAnimatedSprite2D::updateBoidCluster);
...
#define PROPERTY_BINDER(type, name, getter, setter)                                                                    \
    ClassDB::bind_method(D_METHOD(#getter), &BoidAnimatedSprite2D::name);                                              \
    ClassDB::bind_method(D_METHOD(#setter, #name), &BoidAnimatedSprite2D::setter);
